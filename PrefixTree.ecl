...
// The "Create" function macro creates a prefix.
    ...
    Export QueryThorLevenshtein(query_ds, query_field, pt_index, max_edit_distance) := FUNCTIONMACRO
         ...
         QueryPTLayout QueryPTTransform(QueryPTLayout L, RECORDOF(pt_index) R, UNSIGNED1 maxEditDistance) := TRANSFORM
                SELF.query_string         := L.query_string;
                SELF.state                := IF(R.is_word, L.state, CalculateLevenshteinVector(L.query_string, R.node, L.state));

                currentDistance := IF(R.is_word, L.current_distance, GetMinDistance(SELF.state));

                SELF.current_distance     := IF(currentDistance <= maxEditDistance, currentDistance, SKIP);
         ...
         looped_ds := LOOP(prepped_query_ds,
                ...
                JOIN(ROWS(LEFT), pt_index, LEFT.node_id = RIGHT.parent_id AND
                     ...
                     QueryPTTransform(LEFT,RIGHT,max_edit_distance), INNER));
